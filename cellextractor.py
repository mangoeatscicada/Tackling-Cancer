...
tmpdir = tempfile.mkdtemp(prefix='temp.')
...
def main(argv):
    ...
    for i in range(0,len(contours)):
        ...
        if cv2.matchShapes(contours[i], perfect_circle, 1, 0) <= circle_similarity_threshold:
            ...
            if w <= max_size_threshold and h <= max_size_threshold and w >= min_size_threshold and h >= min_size_threshold:
                ...
                cells_found += 1
                #cell_filename = re.sub('\.[^.]*$', "_cell_" + str(cells_found) + ".jpg", os.path.basename(filename))
                cell_filename = re.sub('\.[^.]*$', "_cell_" + str(cells_found) + ".jpg", os.path.join(tmpdir, filename))
                print "filename = ", cell_filename
    ...
    shutil.make_archive('uploads/temp', 'zip', tmpdir)
